...
export const CheckPasswordFormSchema = z
  .object({
    currPassword: z
      .string()
      .trim()
      .min(1, { message: '∙ 현재 비밀번호: 현재 비밀번호를 입력해주세요.' })
      .refine(
        async currPassword => {
        ...
        {
          message: '∙ 현재 비밀번호: 현재 비밀번호가 일치하지 않습니다.',
        },
      ),
    newPassword: z
      .string()
      .trim()
      .min(1, { message: '∙ 새 비밀번호: 새 비밀번호를 입력해주세요.' })
      .regex(/^(?=.*[A-Za-z])(?=.*\d)(?=.*[^\w\s])[A-Za-z\d\S]{8,16}$/, {
        message: '∙ 새 비밀번호: 비밀번호는 8~16자의 영문 대/소문자, 숫자, 특수문자를 조합하여 입력하세요.',
      }),
    checkPassword: z.string().trim().min(1, { message: '∙ 새 비밀번호 확인: 새 비밀번호 확인을 입력해주세요.' }),
  })
  .refine(
    data => {
    ...
    {
      message: '∙ 새 비밀번호 확인: 새 비밀번호가 일치하지 않습니다.',
      path: ['checkPassword'],
    },
  );

...
const Hashtag = ({ hashtag, setHashtag, hashtagArr, setHashtagArr }: { hashtag: string; setHashtag: React.Dispatch<React.SetStateAction<string>>; hashtagArr: string[]; setHashtagArr: React.Dispatch<React.SetStateAction<string[]>> }) => {
  ...
  return (
    <label htmlFor='post_hashtag_input_div' className='post_hashtag_input_div'>
      <input id='post_hashtag_input' className='post_hashtag_input' value={hashtag} onChange={(e) => setHashtag(e.target.value)} onKeyUp={(e) => splitTag(e)} placeholder='#태그를 입력하세요'></input>
    </label>
  );
};

...
const ArticlePrompt = async (keyword: string) => {
  ...
  if (articles.length > 0) {
    ...
    const promptMsg = `You are tasked with analyzing a collection of news articles that share a same keyword. The goal is to identify the central themes that are prevalent across these articles and create a concise and informative blog post summarizing these common key themes.'.
      ...
      Summarize the news articles data provided and write a blog post with one main topic and subheadings. For example, if you are given data for three news articles, merge them into one rather than summarizing each one. Write a blog post in HTML according to the rules set below. Blog posts should be written in Korean and should not contain words that look like news articles, such as 'news', 'article', 'summary', 'theme', or 'overview' in the title or content.
      ...
      5. Include all the contents of the original text, but freely omit unnecessary parts in context, and do not use more than 5 syllables of the same word in the original text to make it appear that it was written by a different person from the original author. This rule must be applied to all rules 1, 2, 3 and 4 written above. This is very important. You can omit the parts that you think are redundant, but as a blogger, please write your blog post as long as possible, even adding your subjective opinion. Here is the news article data.`;

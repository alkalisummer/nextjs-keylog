...
- 인기검색어 및 연관검색어 : 구글 트렌드 API와 네이버 데이터랩 API를 활용하여 실시간 인기 검색어 키워드를 검색량이 많은 순으로 제공하고 키워드 선택시 연관 검색어를 제공합니다.<br>
...
- AutoPossting : 네이버 뉴스 API에서 해당 키워드로 작성된 최근 기사 URL 가져와서 크롤링하고 블로그 게시글을 자동으로 생성합니다. 클립보드 복사 기능을 제공하여 텍스트 에디터
- 이미지 검색 : 네이버 이미지 검색 API를 사용하여 키워드로 검색한 이미지를 무한스크롤로 30개씩 가져옵니다.
